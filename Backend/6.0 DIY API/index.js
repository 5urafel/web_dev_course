...
app.get("/jokes/:id", (req, res) => {
  ...
  let selected_joke = jokes.find((joke) => joke.id == selected_id);
  res.json(selected_joke);
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  let selected_type = req.query.type;
  // let selected_types = req.params.Puns
  let selected_jokes = jokes.filter((joke) => joke.jokeType === selected_type);
  res.json(selected_jokes);
});
...
app.post("/jokes", (req, res) => {
  ...
  jokes.push(newJoke);
  res.send("successfully added a joke!");
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({
      error: `a joke by id number ${id} is not found, so it can't be deleted`,
    });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    if (jokes.length > 0) {
      ...
      res.status(404).json({
        error: `no joke found, so it can't be deleted`,
      });
    }
  } else {
